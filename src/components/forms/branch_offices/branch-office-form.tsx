...
const BranchOfficeForm: FunctionComponent<BranchOfficeForm> = ({
  initialValues,
  loading,
  onSubmit,
  onCancel,
  ...
  const validationSchema = yup.object().shape({
    ...
    latitude: yup.string().max(20).min(5)
    .matches(/^(\+|-)?(?:90(?:(?:\.0{1,6})?)|(?:[0-9]|[1-8][0-9])(?:(?:\.[0-9]{1,6})?))$/, "Ingrese una latitud válida")
    .required(),
    longitude: yup.string().max(20).min(5)
    .matches(/^(\+|-)?(?:180(?:(?:\.0{1,6})?)|(?:[0-9]|[1-9][0-9]|1[0-7][0-9])(?:(?:\.[0-9]{1,6})?))$/, "Ingrese una longitud válida")
    .required(),
  });
    ...
    if (initialValues && countries) {
      ...
      if (countryIndex !== -1) {
        ...
        const stateIndex = countries[countryIndex]?.states?.findIndex(
          (state) => state.name === initialValues.state
        );
        setState(stateIndex);
        setCity(
          countries[countryIndex]?.states[stateIndex]?.cities?.findIndex(
            (city) => city.name === initialValues.city
          )
        );
      }
    }
    ...
    <Form layout={"vertical"} onFinish={handleSubmit(onSubmit)}>
      ...
      <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
        <Col span={12}>
          <Form.Item label={"* Nombre"}>
            <Controller name={"name"} as={Input} control={control} />
            {errors.name && <Text type={"danger"}>{errors.name.message}</Text>}
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item label={"* Email"}>
            <Controller name={"email"} as={Input} control={control} />
            {errors.email && (
      ...
      <Form.Item label={"* Dirección 1"}>
        ...
        {errors.first_address && (
          <Text type={"danger"}>{errors.first_address.message}</Text>
        )}
      </Form.Item>
      <Form.Item label={"Dirección 2"}>
        ...
        {errors.second_address && (
          <Text type={"danger"}>{errors.second_address.message}</Text>
        )}
      </Form.Item>
      <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
        <Col span={8}>
        ...
        <Col span={8}>
          <Form.Item label={"* Provincia"}>
            <Controller
              ...
              options={
                countries &&
                countries[country]?.states?.map((states, index) => ({
                  label: states.name,
                  value: states.name,
                  key: index,
                }))
        ...
        <Col span={8}>
          <Form.Item label={"* Ciudad"}>
            {countries &&
            countries[country]?.states[state]?.cities?.length > 0 ? (
              <Controller
                name={"city"}
                as={Select}
                showSearch
      ...
      <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
        <Col span={12}>
          <Form.Item label={"* Latitud"}>
            <Controller name={"latitude"} as={Input} control={control} />
            {errors.latitude && (
              <Text type={"danger"}>{errors.latitude.message}</Text>
            )}
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item label={"* Longitud"}>
            <Controller name={"longitude"} as={Input} control={control} />
            {errors.longitude && (
              <Text type={"danger"}>{errors.longitude.message}</Text>
            )}
          </Form.Item>
        </Col>
      </Row>
