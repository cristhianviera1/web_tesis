...
class Newness extends Component {
  ...
  getNewness() {
    ...
    const uriText =
      startDate > 0 && endDate > 0
        ? `newness?start_date=${startDate}&end_date=${endDate}`
        : "newness";
    axiosConfig()
      .get(uriText)
      .then(({ data }) => {
  ...
  deleteNewness(newness) {
    axiosConfig()
      .delete(`newness/${newness._id}`)
      .then(() => message.success("Se ha eliminado exitósamente la novedad"))
      .catch((error) => {
        if (error?.response?.data?.message) {
          return message.error(error?.response?.data?.message);
        }
        return message.error(
          "No se pudo eliminar la novedad, por favor intentelo mas tarde"
        );
      })
    ...
    const columns: ColumnsType<NewnessTable> = [
      ...
      },
      {
        ...
        onFilter: (value: any, record) =>
          record.create_date.indexOf(value) === 0,
              ...
              <Space size="large">
                <RangePicker
                  ...
                  onCalendarChange={(currentMoment, value) => {
                    ...
                    this.setState({
                      endDate: moment(value[1]).endOf("day").unix(),
                    });
                  }}
                />
                {
